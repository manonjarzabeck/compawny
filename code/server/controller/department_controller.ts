...
import DepartmentRepository from "../repository/department_repository";

class DepartmentController {
	// méthode reliée à la route en GET située dans le routeur
	public index = async (_req: Request, res: Response) => {
		// récupération des données via le repository
		const results = await new DepartmentRepository().SelectAll();
	...
	public selectOne = async (req: Request, res: Response) => {
		...
		const results = await new DepartmentRepository().SelectOne(req.params);
