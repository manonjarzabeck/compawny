...
import CountryRepository from "../repository/country_repository";

class CountryController {
	// méthode reliée à la route en GET située dans le routeur
	public index = async (_req: Request, res: Response) => {
		// récupération des données via le repository
		const results = await new CountryRepository().SelectAll();
	...
	public selectOne = async (req: Request, res: Response) => {
		...
		const results = await new CountryRepository().SelectOne(req.params);
