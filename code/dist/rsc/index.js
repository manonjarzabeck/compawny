...
function requireReactServerDomWebpackClient_edge_production() {
  ...
  function fulfillReference(reference, value) {
    for (var response = reference.response, handler2 = reference.handler, parentObject = reference.parentObject, key = reference.key, map = reference.map, path = reference.path, i = 1; i < path.length; i++) {
    ...
    for (; "object" === typeof value && null !== value && value.$$typeof === REACT_LAZY_TYPE; )
      if (reference = value._payload, reference === handler2.chunk)
        value = handler2.value;
      else {
    ...
    "" === key && null === handler2.value && (handler2.value = response);
    if (parentObject[0] === REACT_ELEMENT_TYPE && "object" === typeof handler2.value && null !== handler2.value && handler2.value.$$typeof === REACT_ELEMENT_TYPE)
      switch (parentObject = handler2.value, key) {
        case "3":
          parentObject.props = response;
      }
    handler2.deps--;
    0 === handler2.deps && (key = handler2.chunk, null !== key && "blocked" === key.status && (parentObject = key.value, key.status = "fulfilled", key.value = handler2.value, key.reason = handler2.reason, null !== parentObject && wakeChunk(parentObject, handler2.value)));
  }
...
function requireReactServerDomWebpackClient_edge_development() {
  ...
  "production" !== process.env.NODE_ENV && (function() {
    ...
    function fulfillReference(reference, value, fulfilledChunk) {
      ...
      for (; "object" === typeof value && null !== value && value.$$typeof === REACT_LAZY_TYPE; )
        if (path = value._payload, path === handler2.chunk)
          value = handler2.value;
        else {
          switch (path.status) {
            case "resolved_model":
      ...
      "" === key && null === handler2.value && (handler2.value = response);
      if (parentObject[0] === REACT_ELEMENT_TYPE && "object" === typeof handler2.value && null !== handler2.value && handler2.value.$$typeof === REACT_ELEMENT_TYPE)
        switch (reference = handler2.value, key) {
          case "3":
      ...
      handler2.deps--;
      0 === handler2.deps && (fulfilledChunk = handler2.chunk, null !== fulfilledChunk && "blocked" === fulfilledChunk.status && (key = fulfilledChunk.value, fulfilledChunk.status = "fulfilled", fulfilledChunk.value = handler2.value, fulfilledChunk.reason = handler2.reason, null !== key && wakeChunk(key, handler2.value, fulfilledChunk)));
    }
    function rejectReference(reference, error) {
      var handler2 = reference.handler;
      reference = reference.response;
    ...
    function loadServerReference(response, metaData, parentObject, key) {
      ...
      promise.then(
        function() {
          ...
          handler2.deps--;
          0 === handler2.deps && (resolvedValue = handler2.chunk, null !== resolvedValue && "blocked" === resolvedValue.status && (boundArgs = resolvedValue.value, resolvedValue.status = "fulfilled", resolvedValue.value = handler2.value, null !== boundArgs && wakeChunk(boundArgs, handler2.value, resolvedValue)));
        },
        function(error) {
          if (!handler2.errored) {
            var blockedValue = handler2.value;
...
const serverReferences = {
  "7c17d3413dd9": async () => {
    const { $$hoist_0_anonymous_server_function } = await import("./assets/user_action_form_validator-B2x5Jtm4.js");
    return { $$hoist_0_anonymous_server_function };
  },
  "d583bd288879": async () => {
    const { $$hoist_0_anonymous_server_function } = await import("./assets/admin_association_form_validator-4QLA3scJ.js");
    return { $$hoist_0_anonymous_server_function };
  },
  "ffa513e4cd88": async () => {
    const { $$hoist_0_anonymous_server_function } = await import("./assets/admin_action_form_validator-CQow8PYI.js");
    return { $$hoist_0_anonymous_server_function };
  },
  "37e950129265": async () => {
    const { $$hoist_0_anonymous_server_function } = await import("./assets/admin_adoption_form_validator-DjlNTSGE.js");
    return { $$hoist_0_anonymous_server_function };
  }
};
initialize();
...
const NavLink = /* @__PURE__ */ registerClientReference(() => {
  throw new Error("Unexpectedly client reference export 'NavLink' is called on server");
}, "37eafacc3f00", "NavLink");
...
const ScrollRestoration = /* @__PURE__ */ registerClientReference(() => {
  throw new Error("Unexpectedly client reference export 'ScrollRestoration' is called on server");
}, "37eafacc3f00", "ScrollRestoration");
...
function getRouteInstrumentationUpdates(fns, route) {
  ...
  ["loader", "action"].forEach((key) => {
    let handler2 = route[key];
    if (typeof handler2 === "function" && aggregated[key].length > 0) {
      let original = handler2[UninstrumentedSymbol] ?? handler2;
...
function wrapImpl(impls, handler2, getInfo) {
  if (impls.length === 0) {
    return null;
  }
  return async (...args) => {
...
async function recurseRight(impls, info, handler2, index) {
  let impl = impls[index];
  let result;
  if (!impl) {
    try {
      let value = await handler2();
    ...
    let callHandler = async () => {
      if (handlerPromise) {
        ...
        handlerPromise = recurseRight(impls, info, handler2, index - 1);
      }
...
function runServerMiddlewarePipeline(args, handler2, errorHandler) {
...
async function runMiddlewarePipeline(args, handler2, processResult, isResult, errorHandler) {
...
async function callRouteMiddleware(args, middlewares, handler2, processResult, isResult, errorHandler, idx = 0) {
  let { request } = args;
  if (request.signal.aborted) {
  ...
  try {
    ...
    if (lazyHandlerPromise || lazyRoutePromise) {
      if (handler2) {
        ...
        let handler22 = isAction ? match.route.action : match.route.loader;
        if (handler22) {
          [result] = await Promise.all([runHandler(handler22), lazyRoutePromise]);
        } else if (type === "action") {
          let url = new URL(request.url);
...
async function processServerAction(request, basename, decodeReply2, loadServerAction2, decodeAction2, decodeFormState2, onError, temporaryReferences) {
...
async function generateRenderResponse(request, routes, basename, isDataRequest, decodeReply2, requestContext, loadServerAction2, decodeAction2, decodeFormState2, onError, generateResponse, temporaryReferences) {
  let statusCode = 200;
  let url = new URL(request.url);
    ...
    return [
      {
        ...
        lazy: () => import("./assets/root_layout-C1OWxF8d.js"),
        children: [
          {
            id: "public",
            path: "",
            lazy: () => import("./assets/public_layout-BqYSMTxa.js"),
            children: [
              {
                id: "home",
                index: true,
                path: "",
                lazy: () => import("./assets/index-RES3ncnc.js")
              },
              {
                id: "contact",
                path: "contact",
                lazy: () => import("./assets/experience-contact-gzM85cDq.js")
              },
              {
                id: "authentification",
                path: "authentification",
                lazy: () => import("./assets/authentification-DwyZtRdz.js")
              },
              {
                id: "déconnexion",
                path: "logout",
                lazy: () => import("./assets/logout-DzAoNQS_.js")
              },
              {
                id: "espace-utilisateur",
                path: "espace-utilisateur",
                lazy: () => import("./assets/espace-utilisateur-DlfnU_UE.js")
              },
              {
                id: "actions",
                path: "actions",
                lazy: () => import("./assets/action-C162IGC9.js")
              },
              {
                id: "action-detail",
                path: "actions/:id",
                lazy: () => import("./assets/action-details-CUOtdKac.js")
              },
              {
                id: "adoptions",
                path: "adoptions",
                lazy: () => import("./assets/adoptions-sos-BlKBzCru.js")
              },
              {
                id: "adoption-detail",
                path: "adoptions/:id",
                lazy: () => import("./assets/adoption_sos-details-BpmZteEi.js")
              },
              {
                id: "associations",
                path: "associations",
                lazy: () => import("./assets/associations-internationales-D5TA-s9Z.js")
              },
              {
                id: "association-detail",
                path: "associations/:id",
                lazy: () => import("./assets/associations-internationales-details-D2tJI60N.js")
              },
              {
                id: "mentions",
                path: "mentions",
                lazy: () => import("./assets/mentions-legales-BQ5AIwdY.js")
              }
            ]
          },
          {
            id: "admin",
            path: "admin",
            lazy: () => import("./assets/admin_layout-BXe-lGQd.js"),
            children: [
              {
                id: "admin-homepage",
                index: true,
                lazy: () => import("./assets/admin-homepage-CqeF7PHF.js")
              },
              {
                id: "admin-action-homepage",
                path: "action-homepage",
                lazy: () => import("./assets/action_homepage-D2eJ5pyf.js")
              },
              {
                id: "admin-pending-action-homepage",
                path: "pending-action-homepage",
                lazy: () => import("./assets/pending-action-homepage-B8nftpDE.js")
              },
              {
                id: "admin-testimonial-homepage",
                path: "testimonial-homepage",
                lazy: () => import("./assets/testimonial-homepage-RXDKWoGC.js")
              },
              {
                id: "admin-action-form",
                path: "action-form/:id?",
                lazy: () => import("./assets/actions-crud-form-CPIO7ZxZ.js")
              },
              {
                id: "admin-action-form-delete",
                path: "action-form-delete/:id?",
                lazy: () => import("./assets/delete-Dat3mLyU.js")
              },
              {
                id: "admin-association-homepage",
                path: "association-homepage",
                lazy: () => import("./assets/association_homepage-BD4VHIou.js")
              },
              {
                id: "admin-association-form",
                path: "association-form/:id?",
                lazy: () => import("./assets/associations-crud-form-Drr3NkFj.js")
              },
              {
                id: "admin-association-form-delete",
                path: "association-form-delete/:id?",
                lazy: () => import("./assets/delete-YVe3OSnr.js")
              },
              {
                id: "admin-adoption-homepage",
                path: "adoption-homepage",
                lazy: () => import("./assets/adoption_homepage-CbzqnGx3.js")
              },
              {
                id: "admin-adoption-form",
                path: "adoption-form/:id?",
                lazy: () => import("./assets/adoptions-crud-form-CPLLcQ1A.js")
              },
              {
                id: "admin-adoption-form-delete",
                path: "adoption-form-delete/:id?",
                lazy: () => import("./assets/delete-DJQnKCyU.js")
              }
            ]
          }
        ]
...
export {
  ...
  __vite_rsc_react__ as _,
  react_reactServerExports as a,
  requireReact_reactServer as b,
  registerServerReference as c,
  handler as default,
