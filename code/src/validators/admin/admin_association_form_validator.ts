	...
	public validate = async (
		...
		const constraints = z
			.object({
				...
				introduction: z
					.string()
					.max(200, "L'introduction doit comporter, au maximum, 200 caractères")
					.optional()
					.or(z.literal("")),
				website: z
			...
			.refine(
				(data) => {
					if (data.country_id !== 1 && data.department_id) {
						return false;
					}
					return true;
				},
				{
					message: "Un département ne peut être sélectionné que pour la France",
					path: ["department_id"],
				},
			);
